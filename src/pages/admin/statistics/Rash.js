...
const Rash = () => {
    ...
    const getRashData = async () => {
        ...
        await Promise.all(doctors.map(async (item, key) => {
            ...
            let male = 0;
            let rash_request = 0;
            if (item.specialised === "2" || item.specialised === "3") {
                if (item.checkskinsID !== null) {
                    await Promise.all(item.checkskinsID.map(async (checkskin_id) => {
                        ...
                        if (checkskin.type === CheckSkinType.RASH) {
                            rash_request++;
            ...
            data.push({
                ...
                "female": female,
                "male": male,
                "requests": rash_request,
                "portion": ((harmless + insufficient + sucpicious) / rash_request * 100).toFixed(2)
            })
        }))
        let checkskins = await DataStore.query(CheckSkin, (c) => c.type('eq', CheckSkinType.RASH));
